...
function createExpressRouter(config = {}) {
  const {
    yaviRagUrl = DEFAULTS2.yaviRagUrl,
    yaviStructuredUrl = DEFAULTS2.yaviStructuredUrl,
    mongoUri = DEFAULTS2.mongoUri,
    defaultLlmConfig = DEFAULTS2.defaultLlmConfig
  } = config;
  ...
  router.post("/summary-stream", async (req, res, next) => {
    ...
    const separateFlag = separate ?? false;
    const model = yaviLlmConfigName ?? defaultLlmConfig;
    const promptToUse = buildSummaryPrompt2(customPrompt, separateFlag);
    ...
    try {
      const r = await fetch(`${yaviStructuredUrl}/api/v2/session/${sessionId}/chat`, {
        ...
        body: JSON.stringify({
          ...
          add_to_history: add_to_history ?? addToHistory ?? false,
          use_system_prompt: use_system_prompt ?? useSystemPrompt ?? true,
          custom_prompt: custom_prompt ?? customPrompt ?? buildStructuredPrompt2(prompt ?? question)
        })
      });
...
async function connectMongo2(uri) {
  const { MongoClient } = await import("mongodb");
  const client = new MongoClient(uri);
  await client.connect();
  return client;
...
async function getWidgetById(id, mongoUri) {
  const client = await connectMongo2(mongoUri);
  try {
    const { ObjectId } = await import("mongodb");
    const db = client.db();
    const collections = [db.collection("widgets"), db.collection("widgetconfigs")];
...
function buildSummaryPrompt2(custom, separate) {
  if (custom?.trim()) {
...
function buildStructuredPrompt2(custom) {
